...

{
  ...
  auto TMA_load_A_B = [&](int block_start_m, int block_start_n, int bk_idx, int stage)
  {
    if (l == 0)
    {
  ...
  auto store_c = [&](int block_start_m, int block_start_n, float rc[Cfg::acc_per_warp_m][Cfg::acc_per_warp_n][4])
  {
    #pragma unroll
    for (int m = 0; m < Cfg::acc_per_warp_m; m++)
    {
  ...
  int block_start_m = (tile_id % Cfg::GN)*Cfg::BM; 
  int block_start_n = (tile_id / Cfg::GN)*Cfg::BN;
  int outer_base = 0;
  ...
  {
    if (w == 0) TMA_load_A_B(block_start_m,block_start_n,i,i);
  }

  for (int iter = 0; iter < Cfg::persist_num_iters; iter++)
  {
    if (tile_id == -1) break; 
    mbarrier_wait_parity(smem.full(outer_base % Cfg::bk_stages),(outer_base/Cfg::bk_stages) % 2);
    #pragma unroll
    ...
    {
      int bk_cons_stage = (outer_base + bk_idx) % Cfg::bk_stages; 
      int next_bk_cons_stage = (outer_base + bk_idx + 1) % Cfg::bk_stages;
      int parity = ((outer_base + bk_idx + 1) / Cfg::bk_stages) % 2;

      int next_bk_load_idx = (bk_idx + Cfg::bk_stages);
      int next_bk_load_stage = (outer_base + next_bk_load_idx) % Cfg::bk_stages;
      int bk_base = (outer_base) + (bk_idx*Cfg::warp_k_iters);
      ...
      if (w == 0) TMA_load_A_B(block_start_m, block_start_n,next_bk_load_idx,next_bk_load_stage);
    ...
    int next_tile_id = tile_sched::get_linear_tile_id<Cfg::num_tiles, Cfg::num_sms>(iter + 1, b);
    bool has_next = (next_tile_id != -1);

    int store_m = block_start_m;
    int store_n = block_start_n;

    if (has_next)
    {
      block_start_m = (next_tile_id % Cfg::GN) * Cfg::BM;
      block_start_n = (next_tile_id / Cfg::GN) * Cfg::BN;
      outer_base += Cfg::block_k_iters; 
      ...
      for (int i = 0; i < Cfg::bk_stages; i++)
      {
        if (w == 0) TMA_load_A_B(block_start_m, block_start_n, i, (outer_base + i)%Cfg::bk_stages);
      }
    }

    store_c(store_m, store_n, rc);
