          ...
          )
{
  ...
  uint32_t smem_base_a[cfg::bk_stages];
  uint32_t smem_base_b[cfg::bk_stages]; 
  ...
  for (int stage = 0; stage < cfg::bk_stages; stage++)
  {
    ...
    smem_base_a[stage] = static_cast<uint32_t>(__cvta_generic_to_shared(As[stage]));
  }
  ...
  for (int stage = 0; stage < cfg::bk_stages; stage++)
  {
    ...
    smem_base_b[stage] = static_cast<uint32_t>(__cvta_generic_to_shared(Bs[stage]));
  }
  ...
  { 
    for (int iter = 0; iter < cfg::persist_num_iters; iter++)
    {
      ...
      int b_start_m = m*cfg::BM;
      int b_start_n = n*cfg::BN;

      #pragma unroll
      for (int bk_idx = 0; bk_idx < cfg::bk_iters; bk_idx++)
      {
        int stage = (bk_idx + (iter*cfg::bk_iters)) % cfg::bk_stages; 
  ...
  else
  {
    ...
    for (int iter = 0; iter < cfg::persist_num_iters; iter++)
    {
      ...
      uint32_t ra[cfg::wk_iters*cfg::apw_m*4];
      uint32_t rb[cfg::wk_iters*cfg::apw_n*2];
      ...
      for (int bk_idx = 0; bk_idx < cfg::bk_iters; bk_idx++)
      {
        ...
        for (int wk_idx = 0; wk_idx < cfg::wk_iters; wk_idx++)
        {
          ...
          for (int wm_idx = 0; wm_idx < cfg::apw_m; wm_idx++)
          {
            ...
            uint32_t a_smem_addr = smem_base_a[stage]
            + cute_swizzle_byte_offset<cfg::b_bits, cfg::m_base, cfg::s_shift,nv_bfloat16>(a_ld_shared_offset);

            int a_reg_start = ((wm_idx*cfg::wk_iters) + wk_idx)*4; 
            warp_atom::ldmatrix_m8n8_x4_b16(ra[a_reg_start + 0], ra[a_reg_start + 1], ra[a_reg_start + 2], ra[a_reg_start + 3], a_smem_addr);

          }
        }

        for (int wk_idx = 0; wk_idx < cfg::wk_iters/2; wk_idx++)
        {
          #pragma unroll
          for (int wn_idx = 0; wn_idx < cfg::apw_n; wn_idx++)
          {
            int b_ld_shared_offset = (w_start_n + (wn_idx)*cfg::mma_n + (l%8))*cfg::BK + ((2*wk_idx)*cfg::mma_k + 8*(l/8));

            uint32_t b_smem_addr = smem_base_b[stage]
            + cute_swizzle_byte_offset<cfg::b_bits, cfg::m_base, cfg::s_shift,nv_bfloat16>(b_ld_shared_offset);
            int b_reg_start =((wn_idx*(cfg::wk_iters/2))+wk_idx)*4; 
            warp_atom::ldmatrix_m8n8_x4_b16(rb[b_reg_start + 0], rb[b_reg_start + 1],rb[b_reg_start + 2], rb[b_reg_start + 3],b_smem_addr);
          }
        }
        ...
        for (int wk_idx = 0; wk_idx < cfg::wk_iters; wk_idx++)
        {
          ...
          for (int wm_idx = 0; wm_idx < cfg::apw_m; wm_idx++)
          {
            ...
            {
              int a_reg_start = ((wm_idx*cfg::wk_iters) + wk_idx)*4; 
              int b_reg_start = ((wn_idx*cfg::wk_iters) + wk_idx)*2;
...
int main() 
{
  using Cfg = Sm120_BF16_Gemm_Config_swizzle_persist;
  ...
  CUtensorMap a_map =
      TmaDescriptor<nv_bfloat16>::create_2d_row_major(
          A.d_ptr, {Cfg::M, Cfg::K}, {Cfg::BM, Cfg::BK}, Cfg::swizzle_mode);

  CUtensorMap b_map =
      TmaDescriptor<nv_bfloat16>::create_2d_col_major(
          B.d_ptr, {Cfg::K, Cfg::N}, {Cfg::BK, Cfg::BN}, Cfg::swizzle_mode);
...
void print_cfg() {
  using C = Sm120_BF16_Gemm_Config_swizzle_persist;
