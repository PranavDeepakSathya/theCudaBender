...
constexpr uint32_t shared_alloc = ((As_bytes+Bs_bytes)*bk_stages) + (8*1024);
  ...
  else
  {
    ...
    for (int block_iter = 0; block_iter < bm_iters*bn_iters; block_iter++)
    {
      ...
      for (int wm_idx = 0; wm_idx < apw_m; wm_idx++)
      {
        ...
        for (int wn_idx = 0; wn_idx < apw_n; wn_idx++)
        {
          int C_row =  block_m_start +  warp_m_start + (wm_idx*mma_m) + lane_row;
          int C_col = ( block_n_start + warp_n_start + (wn_idx*mma_n) + lane_col)/2;
          int c_reg_start = 4*(wn_idx + apw_n*wm_idx); 
    ...
    int bench_iters  = 100)
{
    for (int i = 0; i < warmup_iters; i++)
        launcher.launch(matmul, a_map, b_map, C);
