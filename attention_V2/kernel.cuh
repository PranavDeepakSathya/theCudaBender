...

{
  ...
  uint32_t kt_frag[Cfg::wlkv_stages][Cfg::D/Cfg::mma_k][Cfg::warp_L_kv/Cfg::mma_n][4]; 
  uint32_t p_frag_packed[Cfg::warp_L_kv/Cfg::mma_k][Cfg::warp_L_q/Cfg::mma_m][4];
  uint32_t v_frag[Cfg::wlkv_stages][Cfg::warp_L_kv/Cfg::mma_k][Cfg::D/Cfg::mma_n][4]; 
  ...
  auto TMA_load_K_V = [&](int blkv_idx, int stage)
  {
    if (t == 0)
    {
      cp_async_bulk_tensor_3d(smem.Ks(stage),&k_map,0,blkv_idx*Cfg::block_L_kv,block_start_BH,smem.KV_bar(stage));
      cp_async_bulk_tensor_3d(smem.Vs(stage),&v_map,blkv_idx*Cfg::block_L_kv,0,block_start_BH, smem.KV_bar(stage));
  ...
  auto load_KT_frag = [&](uint32_t kt_frag[Cfg::wlkv_stages][Cfg::D/Cfg::mma_k][Cfg::warp_L_kv/Cfg::mma_n][4],
  ...
  auto mma_Q_KT = [&](float s_frag[Cfg::warp_L_q/Cfg::mma_m][Cfg::warp_L_kv/Cfg::mma_n][8], 
                      uint32_t q_frag[Cfg::D/Cfg::mma_k][Cfg::warp_L_q/Cfg::mma_m][4],
                      uint32_t kt_frag[Cfg::wlkv_stages][Cfg::D/Cfg::mma_k][Cfg::warp_L_kv/Cfg::mma_n][4],
                      int wlkv_stage)
    ...
    for (int k = 0; k < Cfg::D/Cfg::mma_k; k++)
      {
        ...
        {
          for(int n = 0; n < Cfg::warp_L_kv/Cfg::mma_n; n++)
            wa::mma_m16n16k16_row_col_f32_bf16(s_frag[m][n], q_frag[k][m], kt_frag[wlkv_stage][k][n]);
        }
      }
  };  

  auto scale_softmax_update_pack = [&](float s_frag[Cfg::warp_L_q/Cfg::mma_m][Cfg::warp_L_kv/Cfg::mma_n][8],
                                      uint32_t p_frag_packed[Cfg::warp_L_kv/Cfg::mma_k][Cfg::warp_L_q/Cfg::mma_m][4],
                                      float o_frag[Cfg::warp_L_q/Cfg::mma_m][Cfg::D/Cfg::mma_n][8], 
                                      float m_i[Cfg::warp_L_q/Cfg::mma_m][2],
                                      float l_i[Cfg::warp_L_q/Cfg::mma_m][2])
                        ...
                        int wlkv_idx, int wlkv_stage, int stage)
  {
    ...
    for (int k = 0; k < Cfg::warp_L_kv/Cfg::mma_k;k++)
    {
      ...
      {
        uint32_t v_ld_addr = smem.Vs(stage) + ((n*Cfg::mma_n + ((l%8)+(8*(l/16))))*Cfg::block_L_kv + (wlkv_idx*Cfg::warp_L_kv + k*Cfg::mma_k + (8*((l/8)%2))))*sizeof(nv_bfloat16);
        wa::ldmatrix_m8n8_x4_b16(v_frag[wlkv_stage][k][n],v_ld_addr);
      }
    }
  };
