...
constexpr int warp_n = n_warps/warp_m; 
constexpr int ld_m = 8; 
constexpr int ld_n = 8; 
constexpr int m = warp_m*ld_m; 
constexpr int n = warp_n*ld_n; 
constexpr int n_tpb = n_warps*32;
constexpr int tm = ld_m; 
constexpr int tn = ld_n/2;
...
__global__ void test_ldm_row_maj_x1 (__grid_constant__ const CUtensorMap a_map, __grid_constant__ const CUtensorMap a_out_map)
{
  ...
  int l_m = l/tn; 
  int l_n = (l%tn);
  int w_m_offset = (w_m*ld_m) + l_m;
  int w_n_offset = (w_n*ld_n) + (l_n*2); 
