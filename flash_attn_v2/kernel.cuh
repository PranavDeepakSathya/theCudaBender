...

{
  ...
  if (t == 0)
  {
    ...
    mbarrier_init(k_bar+8,32);
    mbarrier_init(v_bar, 32);
    mbarrier_init(v_bar+8,32);
  ...
  auto TMA_LOAD_K = [&](int blkv,int stage)
  {
    if (w == 0)
    {
      if (l == 0)
      {
        mbarrier_arrive_expect_tx(k_bar + (8*stage), Cfg::Ks_bytes);
        cp_async_bulk_tensor_3d(Ks + (stage*Cfg::Ks_bytes), &k_map, blkv*Cfg::block_L_kv, 0, block_start_BH, k_bar + (8*stage));
      }
      else mbarrier_arrive(k_bar + (8*stage));
    }
    __syncthreads();
  };

  auto TMA_LOAD_V = [&](int blkv, int stage) 
  {
    if (w == 0)
    {
      if (l == 0)
      {
        mbarrier_arrive_expect_tx(v_bar + (8*stage), Cfg::Vs_bytes);
        cp_async_bulk_tensor_3d(Vs + (stage*Cfg::Vs_bytes), &v_map, blkv*Cfg::block_L_kv, 0, block_start_BH, v_bar + (8*stage));
      }
      else mbarrier_arrive(v_bar + (8*stage));

    }
    __syncthreads();
  };
  auto K_smem_rmem = [&](uint32_t kt_frag[Cfg::D/Cfg::mma_k][Cfg::block_L_kv/Cfg::mma_n][4],int stage)
  {
    for (int k = 0; k < Cfg::D/Cfg::mma_k; k++)
    {
      for (int n = 0; n < Cfg::block_L_kv/Cfg::mma_n; n++)
      {
        uint32_t kt_ld_addr = Ks + (stage*Cfg::Ks_bytes) + compact_swizzle<Cfg::swizzle_num>(((k*Cfg::mma_k + (l%16))*Cfg::block_L_kv + (n*Cfg::mma_n + (8*(l/16))))*sizeof(nv_bfloat16));
        wa::ldmatrix_m8n8_x4_trans_b16(kt_frag[k][n], kt_ld_addr);
      }
    }
  };

  auto V_smem_rmem = [&](uint32_t v_frag[Cfg::block_L_kv/Cfg::mma_k][Cfg::D/Cfg::mma_n][4], int stage)
  {
    for (int k = 0; k < Cfg::block_L_kv/Cfg::mma_k;k++)
    {
      for (int n = 0; n < Cfg::D/Cfg::mma_n; n++)
      {
        uint32_t v_ld_addr = Vs + (stage*Cfg::Vs_bytes) + compact_swizzle<Cfg::swizzle_num>(((n*Cfg::mma_n + ((l%8)+(8*(l/16))))*Cfg::block_L_kv + (k*Cfg::mma_k + (8*((l/8)%2))))*sizeof(nv_bfloat16));
  ...
  {
    int next_blkv = blkv+1; 
    int next_stage = next_blkv % 2; 
    int stage = blkv % 2; 
    float s_frag[Cfg::warp_L_q/Cfg::mma_m][Cfg::block_L_kv/Cfg::mma_n][8] = {0.0}; 
    __syncthreads(); 
    TMA_LOAD_K(next_blkv,next_stage); 
    TMA_LOAD_V(next_blkv,next_stage);

    mbarrier_wait_parity(k_bar +(8*stage), k_parity);
    K_smem_rmem(kt_frag,stage);
    mma_Q_KT_scale(s_frag, q_frag, kt_frag);
    flash_softmax_update<Cfg>(s_frag, o_frag, m_i, l_i);
    pack_p_frag<Cfg>(s_frag, p_frag_packed);
    mbarrier_wait_parity(v_bar + (8*stage), v_parity);
    V_smem_rmem(v_frag,stage);
