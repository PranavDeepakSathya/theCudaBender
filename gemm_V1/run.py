...
# hash of bench.cu so stale .so files are never reused after source changes
...
def cfg_hash(cfg_vals):
    return hashlib.md5(str(((M, N, K), cfg_vals, _bench_hash)).encode()).hexdigest()[:12]

def so_path(h):
    return str(BUILD_DIR / f"gemm_{h}.so")

def compile_one(item):
    cfg_vals, src, cuda_inc, atoms_inc, nvcc_bin = item
    h     = cfg_hash(cfg_vals)
    out   = so_path(h)
    if Path(out).exists():
        return cfg_vals, h, None   # already built

    flags = cfg_to_flags(cfg_vals)
    cmd = [
        nvcc_bin,
...
work = [(c, BENCH_SRC, CUDA_INC, ATOMS_INC, NVCC) for c in combos]
...
TensorMap = ctypes.c_uint8 * 128   # CUtensorMap is 128 bytes

torch.manual_seed(0)
device = "cuda"
A    = torch.randn((M, K), device=device, dtype=torch.bfloat16).contiguous()
B    = torch.randn((N, K), device=device, dtype=torch.bfloat16).t().contiguous()
...
for i, (cfg_vals, h) in enumerate(compiled, 1):
    ...
    a_ptr_ct = ctypes.cast(a_map, ctypes.c_void_p)
    b_ptr_ct = ctypes.cast(b_map, ctypes.c_void_p)
    ...
    try:
        ms = do_bench(
            lambda: lib.run_gemm(A_ptr, B_ptr, C_ptr, a_ptr_ct, b_ptr_ct),
            return_mode="median"
        )
