...
constexpr int num_mma_k_iters = 4; 
...

{
  ...
  for (bk = 0; bk < num_BK_iters-1; bk++)
  {
    int curr_stage = bk % 2; 
    int next_stage = (bk + 1) % 2; 
    int next_block_k_start = (bk+1)*BK; 

    //pre_fetch_next_stage
    if (is_elected())
    {
      

      int32_t coords_A[2] = {next_block_k_start,block_start_m};
      int32_t coords_B[2] = {next_block_k_start,block_start_n}; 
    ...
    // wait on the current stage before computing 
    ...
    for (int wk = 0; wk < num_mma_k_iters; wk++)
    {
      for (int wm = 0; wm < acc_per_warp_m; wm++)
      {
        ...
        uint32_t a_ld_addr = (smem_base_a[curr_stage]) + ((a_load_shared_col + (BK*a_load_shared_row))*sizeof(nv_bfloat16));
      ...
      for (int wn = 0; wn < acc_per_warp_n; wn++)
      {
        ...
        uint32_t  b_ld_addr = (smem_base_b[curr_stage]) + ((b_load_shared_row + (BK*b_load_shared_col))*sizeof(nv_bfloat16)); 
