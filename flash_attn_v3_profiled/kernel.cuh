...

{
  ...
  IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kLoadQ, 0, ikp_on);
  if (w == 0)
  {
    if (t == 0)
    {
  ...
  __syncthreads();
  IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kLoadQ, 1, ikp_on);
  ...
  for (int blkv = 0; blkv < (Cfg::L_kv/Cfg::block_L_kv) - 1; blkv++)
  {
    ...
    IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kLoadK_smem, 1, ikp_on);

    TMA_LOAD_K(blkv+1); //2 TMAs in flight

    IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kComputeQK, 0, ikp_on);
    mma_Q_KT_scale(s_frag, q_frag, kt_frag); // one set of MMAs in flight
    IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kComputeQK, 1, ikp_on);

    IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kSoftmax, 0, ikp_on);
    ...
    V_smem_rmem(v_frag);
    IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kLoadV_smem, 1, ikp_on);

    IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kComputePV, 0, ikp_on);
    mma_P_V(o_frag,p_frag_packed,v_frag);
    IKP_TRACE_REC_IF(ikp_ctx, ikp_buf, kComputePV, 1, ikp_on);
...
inline void launch_attention(
    ...
    ikp::GlobalBuffer ikp_buf)
{
    launcher.launch(attention_kernel<Cfg>, q_map, k_map, v_map, O, ikp_buf);
}