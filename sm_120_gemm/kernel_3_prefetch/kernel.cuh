...
)
{
  ...
  if (w == 0)
  {
    if(l == 0)
      {
        mbarrier_arrive_expect_tx(bar(stage),Cfg::As_bytes + Cfg::Bs_bytes);
        gemm.load_A_g2s(bk_idx, block_start_m, As(stage), full_bar(stage));
        gemm.load_B_g2s(bk_idx, block_start_n, Bs(stage), full_bar(stage)); 
      }
      else
      {
        mbarrier_arrive(bar(stage));
      }
  }
  
  mbarrier_wait_parity(bar,wait_parity); 
  __syncthreads(); 

  for (bk_idx = 0; bk_idx < Cfg::block_k_iters - (Cfg::bk_stages-1); bk_idx++)
  {
