...
__global__ void test_ldm_row_maj_x1 (__grid_constant__ const CUtensorMap a_map, __grid_constant__ const CUtensorMap a_out_map)
{
  ...
  int w_m = w/warp_n; 
  int w_n = w % warp_n; //NOTICCE HOW WE HAVE LEFT THE WARP LAYOUT ROW MAJOR 
  // if we want contigous warps then we must go to warp layout COL MAJOR 
...
//for column major, the lane id must give the addresses of columns
// that is LANE LAYOUT MUST BE COL MAJOR 

  int my_row_idx = w_m_offset; 
  int my_col_idx = w_n_offset + (lane % 8);
