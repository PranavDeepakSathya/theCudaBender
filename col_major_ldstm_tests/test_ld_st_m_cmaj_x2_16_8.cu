...
constexpr int ld_m = 16;        // 16 rows per warp
constexpr int ld_n = 8;         // 8 cols per warp
...
__global__ void test_ldm_row_maj_x2 (__grid_constant__ const CUtensorMap a_map, __grid_constant__ const CUtensorMap a_out_map)
{
  ...
  int lane_row_offset = lane_n*(lane / 8); //(only the first 16 lanes participate so we guuci, this is effectively 0)
  int lane_col_offset = lane % 8;  
