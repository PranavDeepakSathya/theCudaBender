...
enum ProfilerTag {
  TAG_Q_GMEM_TO_SMEM = 0,
  TAG_Q_GMEM_TO_RMEM,
  TAG_KV_GMEM_TO_SMEM,
  TAG_K_SMEM_TO_RMEM,
  TAG_QK_MMA,
  TAG_SOFTMAX_AND_UPDATE,
  TAG_SCALE,
  TAG_V_SMEM_TO_RMEM,
  TAG_PV_MMA,
  TAG_STORE,
...

{
  ...
  uint32_t Qs = smem;
  uint32_t Ks = smem;
  uint32_t Vs = smem + (Cfg::Ks_total);
  uint32_t Qs_bar = smem + Cfg::bar_start_offset;
  uint32_t KVs_bar = Qs_bar + 8;
  ...
  // ---- profiler: only block 0, warp-level ----
  const bool do_prof = (b == 0);
  Profiler prof;
  if (do_prof && is_elected())
    prof.init(num_prof_entries, prof_buf, w);
  ...
  // ---- Q: gmem -> smem (TMA) ----
  if (do_prof && w == 0 && is_elected()) prof.start(TAG_Q_GMEM_TO_SMEM);
  ...
  if (do_prof && w == 0 && is_elected()) prof.stop();

  // ---- Q: smem -> rmem (ldmatrix) ----
  if (do_prof && is_elected()) prof.start(TAG_Q_GMEM_TO_RMEM);
  ...
  if (do_prof && is_elected()) prof.stop();

  int parity = 0;

  auto TMA_LOAD_K_V = [&](int blkv, int stage)
  {
    if (do_prof && w == 0 && is_elected()) prof.start(TAG_KV_GMEM_TO_SMEM);
  ...
  auto consume = [&](int stage)
  {
    for (int wlkv = 0; wlkv < Cfg::block_L_kv/Cfg::warp_L_kv; wlkv++)
    {
      ...
      // ---- K: smem -> rmem ----
      if (do_prof && is_elected()) prof.start(TAG_K_SMEM_TO_RMEM);

      for (int k = 0; k < Cfg::D/Cfg::mma_k; k++)
      {
        for (int n = 0; n < Cfg::warp_L_kv/Cfg::mma_n; n++)
        {
          uint32_t kt_ld_addr = Ks + (stage*Cfg::Ks_bytes) +  compact_swizzle<Cfg::D_swizzle_num>(((wlkv*Cfg::warp_L_kv + n*Cfg::mma_n + ((l%8)+(8*(l/16))))*Cfg::D + (k*Cfg::mma_k + (8*((l/8)%2))))*sizeof(nv_bfloat16));
      ...
      // ---- V: smem -> rmem ----
      if (do_prof && is_elected()) prof.start(TAG_V_SMEM_TO_RMEM);

      for (int k = 0; k < Cfg::warp_L_kv/Cfg::mma_k;k++)
      {
        for (int n = 0; n < Cfg::D/Cfg::mma_n; n++)
        {
          uint32_t v_ld_addr = Vs + (stage*Cfg::Vs_bytes) + compact_swizzle<Cfg::swizzle_num>(((n*Cfg::mma_n + ((l%8)+(8*(l/16))))*Cfg::block_L_kv + (wlkv*Cfg::warp_L_kv + k*Cfg::mma_k + (8*((l/8)%2))))*sizeof(nv_bfloat16));
      ...
      // ---- QK MMA ----
      if (do_prof && is_elected()) prof.start(TAG_QK_MMA);
      ...
      // ---- scale ----
      if (do_prof && is_elected()) prof.start(TAG_SCALE);
      ...
      // ---- softmax + online update + pack ----
      if (do_prof && is_elected()) prof.start(TAG_SOFTMAX_AND_UPDATE);
      ...
      // ---- PV MMA ----
      if (do_prof && is_elected()) prof.start(TAG_PV_MMA);
  ...
  for (int s = 0; s < Cfg::kv_stages-1; s++)
  {
    TMA_LOAD_K_V(s,s);
  }

  for (int blkv = 0; blkv < (Cfg::L_kv/Cfg::block_L_kv) - (Cfg::kv_stages-1); blkv++)
  {
    int next_load_idx = blkv + Cfg::kv_stages - 1;
    int next_load_stage = next_load_idx % Cfg::kv_stages;
  ...
  for (int blkv = (Cfg::L_kv/Cfg::block_L_kv) - (Cfg::kv_stages-1); blkv < (Cfg::L_kv/Cfg::block_L_kv); blkv++)
  {
    int current_consume_stage = blkv % Cfg::kv_stages;
    int current_consume_parity = (blkv /Cfg::kv_stages) % 2;
    __syncthreads();
  ...
  if (do_prof && is_elected()) prof.start(TAG_STORE);
  ...
  if (do_prof && is_elected()) prof.flush();
}
...
inline void launch_attention(
    ...
    int num_prof_entries)
{
    launcher.launch(attention_kernel<Cfg>, q_map, k_map, v_map, O, prof_buf, num_prof_entries);
}
